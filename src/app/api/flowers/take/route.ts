  ...
  try {
    ...
    if (Array.isArray(body)) {
      ...
      for (const flower of body) {
        if (success == 'true') {
          const existingFlower = await db.select().from(flowers).where(eq(flowers.name, flower.name));
          if (existingFlower.length > 0 && existingFlower[0] && existingFlower[0].quantity >= flower.quantity) {
      ...
      return NextResponse.json({ message: (success == 'true' ? "Flower updated successfully" : "Flower does not exist or insufficient quantity"), success: success });
    } else {
      ...
      if (existingFlower.length > 0 && existingFlower[0] && existingFlower[0].quantity >= body.quantity) {
        // If flower exists, update its quantity
        await db.update(flowers)
          .set({ quantity: existingFlower[0].quantity - body.quantity })
          .where(eq(flowers.name, body.name));
      } else {
        return NextResponse.json({ message: 'Flower does not exist or insufficient quantity' });
      }
    }
    ...
    return NextResponse.json({ error: 'Failed to take flower' }, { status: 500 });
  }
} 